...
export default function NewsCard({ commodityId }: { commodityId: string }) {
  ...
  useEffect(() => { fetchNews(); }, [commodityId]);
        ...
        <ol className="space-y-3">
          {articles.map((article, i) => (
            <li key={i} className="flex gap-3 group">
              <span
                className="text-xs font-mono font-bold mt-0.5 shrink-0 w-4"
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  ...
                  <ExternalLink size={10} className="mt-1 shrink-0 opacity-0 group-hover:opacity-60 transition-opacity" style={{ color: 'var(--muted)' }} />
                </a>
                <div className="flex items-center gap-1.5 mt-1">
                  <span className="text-[10px]" style={{ color: 'var(--accent)' }}>{article.source.name}</span>
                  <span style={{ color: 'var(--border)' }}>·</span>
                  <span className="text-[10px]" style={{ color: 'var(--muted)' }}>{formatDate(article.publishedAt)}</span>
                </div>
              </div>
            </li>
          ))}
